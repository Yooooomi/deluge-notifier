...
class Gtk3UI(Gtk3PluginBase):
    def enable(self):
        ...
        component.get('Preferences').add_page(
            'MyPlugin', self.builder.get_object('prefs_box'))
    ...
    def disable(self):
        component.get('Preferences').remove_page('MyPlugin')
    ...
    def on_apply_prefs(self):
        log.debug('applying prefs for MyPlugin')
        config = {
            'test': self.builder.get_object('txt_test').get_text()
        }
        client.myplugin.set_config(config)

    def on_show_prefs(self):
        client.myplugin.get_config().addCallback(self.cb_get_config)
